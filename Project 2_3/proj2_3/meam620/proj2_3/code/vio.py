...
def error_covariance_update(nominal_state, error_state_covariance, w_m, a_m, dt,
    ...
    # broadcast the state
    calibrated_acceleration = (a_m - a_b).flatten()
    acceleration_skew_symmetrical_matrix = np.array([
        [0, -calibrated_acceleration[2], calibrated_acceleration[1]],
        [calibrated_acceleration[2], 0, -calibrated_acceleration[0]],
        [-calibrated_acceleration[1], calibrated_acceleration[0], 0]
    ]) # skew symmetrical form of acceleration vector

    # linearization
    Fx = np.identity(18)
    Fx[0:3,3:6] = dt * np.identity(3)
    Fx[3:6, 6:9] = - q.as_matrix() @ acceleration_skew_symmetrical_matrix * dt
